...
from ci.jobs.scripts.integration_tests_configs import IMAGES_ENV, PARALLEL_SKIP_TESTS
...
def main():
    ...
    test_files.sort()
    parallel_skip_prefixes = sorted(list(PARALLEL_SKIP_TESTS))
    ...
    if args.test:
        ...
        if parallel_test_modules:
            test_result_parallel = Result.from_pytest_run(
                command=f"{' '.join(sorted(parallel_test_modules))} --report-log-exclude-logs-on-passed-tests -n {workers} --dist=loadfile --tb=short {repeat_option} --timeout=0",  # disable xdist 3300s timeout
                cwd="./tests/integration/",
                env=test_env,
            ...
            files.append(f"{temp_path}/pytest_parallel.jsonl")
        ...
        if sequential_test_modules and fail_num < MAX_FAILS_BEFORE_DROP:
            test_result_sequential = Result.from_pytest_run(
                command=f"{' '.join(sorted(sequential_test_modules))} --report-log-exclude-logs-on-passed-tests --tb=short {repeat_option} --timeout=0 -p no:xdist",  # disable xdist 3300s timeout
                cwd="./tests/integration/",
                env=test_env,
            ...
            files.append(f"{temp_path}/pytest_sequential.jsonl")
